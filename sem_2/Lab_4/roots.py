...
def methodTrapec(a,b,n,k,func):
    ...
    while a1<b:
      summa+=func(a1)
      a1+=h
    Ploshad = h*((func(a)+func(b))/2 + summa)
    print('Площадь ',k,'= ',round(abs(Ploshad),6))
...
print('-------------------------------------------------------------------------')
print('|№ корня |  A  |   B  |  корень(х) |    f(x)   |Число итераций| Ошибка |')
print('-------------------------------------------------------------------------')
...
if len(root)!=0:
   ...
   plt.legend(loc='upper left')
   if len(root)>1:
      print('ЧАСТЬ 2:\nВычисление площади методом <Трапеция>')
      for i in range(1,len(root)):
           methodTrapec(root[i-1],root[i],100,i,Function1)
           x = np.linspace(root[i-1],root[i],1000)
           plt.fill_between(x,x**2-2*x-3, np.sin(x),label = '$Площади$')
   else:
      print('Вычисление площади невозможно: нужно как минимум 2 точки пересеч.')
   plt.show()
